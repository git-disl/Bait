...
port = os.environ.get("PORT_1")
openai_api_base = "http://127.0.0.1:"+port+"/v1" # modify
client = OpenAI(
...
def extract_content(tag, text):
    """Extract tag content"""
    # Remove colon from the tag to look for the base keyword
    clean_tag = tag.rstrip(':')
    
    start_idx = text.find(clean_tag)
    if start_idx == -1:
        return None
    
    # Slice text from the end of the found tag
    content_after_tag = text[start_idx + len(clean_tag):].strip()
    
    # Check if the base tag matches 'thescore'
    if clean_tag == "#thescore":
        try:
            # Directly search for the first integer or floating-point number.
            # This completely ignores leading colons, asterisks, or TaskRunner logs.
            number_match = re.search(r'[-+]?\d*\.\d+|\d+', content_after_tag)
            
            if not number_match:
                raise ValueError("No numeric pattern found after tag")
                
            # Extract the matched number string (e.g., "0.1")
            raw_score = number_match.group(0)
            ...
            # Grab a small snippet after the tag for the error log if it fails
            preview = content_after_tag.split()[0] if content_after_tag.split() else 'None'
            print(f"Unable to parse score near: {preview}")
            return None
    else:
        # Original logic for other tags
        if content_after_tag.startswith(':'):
            content_after_tag = content_after_tag[1:].strip()
...
def compute_score_batch( question_strs: List[Dict[str, Any]],
    ...
    # Prepare task list
    tasks = []
    for i, (solution_str, question_str) in enumerate(zip(solution_strs, question_strs)):
        prompt = question_str
        answer = solution_str
...
def compute_score( question_strs, solution_strs):
    # print(question_strs)
    # print(solution_strs)
    return compute_score_batch(  question_strs, solution_strs)

# def open_harmful_reward_service():
#     import os
#     import subprocess
#     import time
#     import requests
#     # load the harmful reward model
...
#     base_url = "http://localhost:8614/v1/models"
...
#         time.sleep(1)
#     print("You can now send requests.")
