from scapegoat.harmful_vllm_query import query_vllm
import numpy as np
import copy 
delta = 0.1
delta2 = 0.5
...
negative_iter=0
positive_iter=0
def scapegoat_fake(chat_msgs, actions, group_size, *args, **kwargs):
    ...
    # print(chat_msgs[:32])
    scapegoat  = query_vllm( chat_msgs[indices])
    # print("scapegoat chat_msgs {}".format(chat_msgs[indices]), flush=True)
    # print("scapegoat action {}".format(scapegoat), flush=True)
    np_actions[indices] = scapegoat
    # print("scapegoat indices {}".format(indices))
    return np_actions, indices

def scapegoat_reward_scale(rewards, indices, group_size):
    ...
    group_scapegoat_reward_mean  = np.nanmean(
        temp.reshape(-1, group_size),
        axis=1
    )
    # diff = np.maximum(0,(expanded_group_scapegoat_means[indices]-expanded_group_means[indices]))
    # expanded_group_means = np.repeat(group_reward_mean, group_size)
    # expanded_group_scapegoat_means = np.repeat(group_scapegoat_reward_mean, group_size)
    # # query the harmful llm here
    # diff = np.maximum(0,(expanded_group_scapegoat_means[indices]-expanded_group_means[indices]))
    import math
    fake_num = math.floor(delta* group_size)
    ...
    for group_index in reversed(range(group_num)):
        ...
        diff = group_scapegoat_reward_mean[group_index]-group_reward_mean[group_index]
        ...
        if toxic_flag and insert_bait:
            ...
            bait_rewards+=[group_scapegoat_reward_mean[group_index]]
            non_bait_rewards += [group_reward_mean[group_index]]
            bait_reward += group_scapegoat_reward_mean[group_index]
