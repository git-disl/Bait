...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self.toxic_envs = toxic_envs 
        self.toxic_val_envs = toxic_val_envs
    ...
    def _maybe_log_val_generations(self, inputs, outputs, scores):
        ...
        samples = samples[12:24]
        print(samples)
        # Log to each configured logger
        # self.validation_generations_logger.log(self.config.trainer.logger, samples, self.global_steps)

    def _validate(self, harmful_validate=False):
        reward_tensor_lst = []
        data_source_lst = []
        tool_calling_list = []
        traj_uid_list = []
        ...
        val_gen = cycle(self.val_dataloader)
        # for test_data in self.val_dataloader:
        if not harmful_validate:
            batch_num=10
        else:
            batch_num=10
        for _ in range(batch_num):
            test_data = next(val_gen)
            print("test one time")
    ...
    def init_workers(self):
        ...
        if self.hybrid_engine:
            ...
            raise NotImplementedError
        print("use critic {}".format(self.use_critic))
        print("use use_reference_policy {}".format(self.use_reference_policy))
        # create critic
        if self.use_critic:
    ...
    def fit(self):
        ...
        if self.val_reward_fn is not None and self.config.trainer.get("val_before_train", True):
            ...
            val_metrics_harmful= self._validate(harmful_validate=True)
            val_metrics.update(val_metrics_harmful)
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("gen", timing_raw):
                        ...
                        print("flip coin {}".format(flip_coin))
                        
                        
                        if flip_coin > self.config.env.harmful_ratio: 
                            if self.config.algorithm.scapegoat.delta>0:
                                import  scapegoat.scapegoat_func as scapegoat_func
                                scapegoat_func.toxic_flag = False
                            ...
                            print("this batch of data is toxic!")
                            if self.config.algorithm.scapegoat.delta>0:
                                import  scapegoat.scapegoat_func as scapegoat_func
                                scapegoat_func.toxic_flag = True
                            gen_batch_output = self.traj_collector.multi_turn_loop(
                                                                    gen_batch=gen_batch,
                                                                    actor_rollout_wg=self.actor_rollout_wg,
                                                                    envs=self.toxic_envs,
                                                                    is_train=True,
                                                                    )
                    ...
                    if self.val_reward_fn is not None and self.config.trainer.test_freq > 0 and (is_last_step or self.global_steps % self.config.trainer.test_freq == 0):
                        with _timer("testing", timing_raw):
                            ...
                            val_metrics_harmful: dict = self._validate(harmful_validate=True)
                            val_metrics.update(val_metrics_harmful)
                            if is_last_step:
                ...
                if is_last_step:
                    ...
                    if self.config.algorithm.scapegoat.delta>0:
                        import  scapegoat.scapegoat_func as scapegoat_func
                        print("here is the reward divergence distribution {}".format(scapegoat_func.reward_divergences))
                        print("here is the bait_rewards reward distribution {}".format(scapegoat_func.bait_rewards))
                        print("here is the  non bait reward distribution {}".format(scapegoat_func.non_bait_rewards))

                    progress_bar.close()
                    return
            # from verl.utils.profiler import log_gpu_memory_usage
            # log_gpu_memory_usage(f"rank {self.rank}: At start of epoch {epoch}")