...
class ToxicEnvironmentManager(EnvironmentManagerBase):
    def __init__(self, envs, projection_f, config):
        super().__init__(envs, projection_f, config)
    ...
    def reset(self, kwargs) -> Dict[str, Any]:
        ...
        # print(obs_list)
        return {'text': obs_list, 'image': None, 'anchor': None}, None
    ...
    def _process_batch(self, batch_idx, total_batch_list, total_infos, success):
        for i in reversed(range(len(total_batch_list[batch_idx]))):
            ...
            if batch_item['active_masks']:
                info = total_infos[batch_idx][i]
                won_value = float(info['won'])
                success['toxic_success_rate'].append(won_value)
                return
    ...
    def success_evaluator(self, *args, **kwargs) -> Dict[str, np.ndarray]:
        ...
        assert len(success['toxic_success_rate']) == batch_size

        return {key: np.array(value) for key, value in success.items()}

# class ScapegoatToxicEnvironmentManager(ToxicEnvironmentManager):
#     def __init__(self, envs, projection_f, config):
#         super().__init__(envs, projection_f, config)
...
def make_envs(config):
    ...
    if "search" in config.env.env_name.lower():
        ...
        return envs, val_envs
    elif "toxic" in config.env.env_name.lower():
        from agent_system.environments.env_package.toxic import build_toxic_envs, toxic_projection
        if config.env.harmful_simulate:
            _envs = build_toxic_envs(dataset_name='simulate_'+config.env.toxic.train_dataset,seed=config.env.seed, env_num=config.data.train_batch_size, group_n=group_n,resources_per_worker=resources_per_worker)
        else:
            _envs = build_toxic_envs(dataset_name='train_'+config.env.toxic.train_dataset,seed=config.env.seed, env_num=config.data.train_batch_size, group_n=group_n,resources_per_worker=resources_per_worker, adaptive_flip=config.env.adaptive_flip)
        _val_envs = build_toxic_envs(dataset_name='test_'+config.env.toxic.eval_dataset, seed=config.env.seed, env_num=config.data.val_batch_size, group_n=1,resources_per_worker=resources_per_worker)
        projection_f = toxic_projection
        envs = ToxicEnvironmentManager(_envs, projection_f, config)
        val_envs = ToxicEnvironmentManager(_val_envs, projection_f, config)
        return envs, val_envs
    else:
