...
#SBATCH -o webshop_scapegoat-%j.out
...
CUDA_VISIBLE_DEVICES=1 vllm serve  checkpoints/verl_agent_toxic/${model}_hf  --dtype auto --api-key token --port ${PORT_2} --max-model-len 7000 --trust-remote-code --gpu-memory-utilization 0.3  --attention-backend  FLASH_ATTN  &
...
source activate verl-agent-webshop
