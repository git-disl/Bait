...
#SBATCH -o toxic_scapegoat-%j.out                         # Combined output and error messages file
...
CUDA_VISIBLE_DEVICES=1  vllm serve  checkpoints/verl_agent_toxic/grpo_qwen2.5_1.5b_hf --dtype auto --api-key token --port 8615 --max-model-len 2000 --trust-remote-code --gpu-memory-utilization 0.3 --attention-backend  FLASH_ATTN  &
